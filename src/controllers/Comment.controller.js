...
exports.findAll = (req, res) => {
    ...
    Comment.findAndCountAll({
        attributes: ['id', 'comment', 'date'], where: condition, limit, offset,
        include: [
            {
...
exports.update = async (req, res) => {
    ...
    if (!comment)
        return res.status(404).json({ message: `Not found comment with id=${req.params.commentID}.` });

    if (comment.id_user == req.loggedUserId || user.id_type == 1) {
